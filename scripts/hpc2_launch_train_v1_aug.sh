#!/bin/bash
#SBATCH --job-name=l2am_v1_aug  # 任务名称
#SBATCH -p i64m1tga800u                     # 指定GPU分区（根据HPC文档，可替换为实际可用分区如normal等）
#SBATCH --output=l2am_v1_aug_%j.out      # 主日志
#SBATCH --error=l2am_v1_aug_%j.err       # 错误日志
#SBATCH --nodes=2                        # 单节点
#SBATCH --gres=gpu:6                     # 单张GPU（80G）
#SBATCH --cpus-per-task=16               # 足够CPU核心支持并行
#SBATCH --mem=80G                        # 充足内存
