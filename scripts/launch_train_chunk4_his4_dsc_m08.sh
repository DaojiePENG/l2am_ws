...
# 用法: scripts/launch_train_chunk4_his4_dsc_m08.sh <id> [suffix]
# 例如: scripts/launch_train_chunk4_his4_dsc_m08.sh 01 chunk4_his4_dsc_m08
...
nohup bash -c '
    torchrun \
        --nproc_per_node=4 \
        --master_port=29505 \
        l2am/train_chunk4_his4_dsc_m08.py \
        > "'"$LOG_FILE"'" 2> "'"$ERR_FILE"'"
...
PYTHON_PID=""
# 方法：查找 WRAPPER_PID 的子进程中包含 "train_chunk4_his4_dsc_m08.py" 的 python 进程
while read -r pid ppid cmd; do
    if [[ "$ppid" == "$WRAPPER_PID" ]] && [[ "$cmd" == *"python"* ]] && [[ "$cmd" == *"train_chunk4_his4_dsc_m08.py"* ]]; then
        PYTHON_PID="$pid"
        break
    fi
