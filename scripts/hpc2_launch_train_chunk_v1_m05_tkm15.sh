#!/bin/bash
#SBATCH --job-name=l2am_chunk_v1_m05_tkm15  # 任务名称
#SBATCH -p i64m1tga800u                     # 指定GPU分区（根据HPC文档，可替换为实际可用分区如normal等）
#SBATCH --output=l2am_chunk_v1_m05_tkm15_%j.out      # 主日志
#SBATCH --error=l2am_chunk_v1_m05_tkm15_%j.err       # 错误日志
#SBATCH --nodes=1                        # 单节点
#SBATCH --gres=gpu:1                     # 单张GPU（80G）
#SBATCH --cpus-per-task=48               # 足够CPU核心支持并行
#SBATCH --mem=80G                        # 充足内存
...
$PYTHON -m torch.distributed.run \
    ...
    --master_port=29500 \
    l2am/train_chunk_v1_m05_tkm15.py