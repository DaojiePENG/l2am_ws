...
CACHE_DIR = "data/cache/train_frames_chunk4_his1_s_m08_rxr"
VAL_DATA_DIR = "data/l2am_rxr_v3/val_seen/6"
VAL_CACHE_DIR = "data/cache/val_seen_frames_chunk4_his1_s_m08_rxr"
VAL_U_DATA_DIR = "data/l2am_rxr_v3/val_unseen/6"
VAL_U_CACHE_DIR = "data/cache/val_unseen_frames_chunk4_his1_s_m08_rxr"
...
# training configs
OUTPUT_DIR = "outputs/l2a_bigbird_action_classifier_chunk4_his1_s_m08_rxr"
NUM_EPOCHS = 25
PER_DEVICE_TRAIN_BATCH_SIZE = 18
PER_DEVICE_EVAL_BATCH_SIZE = 198
GRADIENT_ACCUMULATION_STEPS = 1
LEARNING_RATE = 8e-5
WARMUP_RATIO = 0.08  # 学习率预热比例
WANDB_RUN_NAME = "bigbird-action_chunk4_his1_s_m08_rxr"  # 可选：设置 wandb 实验名称
LOGGING_STEPS = 100
EVAL_STEPS = 1000
SAVE_STEPS = 1000
...
# 2. 加载或预处理数据集
# ======================
from dataset_utils_final import get_or_create_dataset_chunk_s_his
...
def main():
    ...
    # Step 1: 获取帧级数据集
    ds = get_or_create_dataset_chunk_s_his(DATA_DIR, CACHE_DIR, num_grid_r=NUM_GRID_R, num_grid_c=NUM_GRID_C, num_chunk=NUM_CHUNK, num_his=num_his)
    vds = get_or_create_dataset_chunk_s_his(VAL_DATA_DIR, VAL_CACHE_DIR, num_grid_r=NUM_GRID_R, num_grid_c=NUM_GRID_C, num_chunk=NUM_CHUNK, num_his=num_his)
    vuds = get_or_create_dataset_chunk_s_his(VAL_U_DATA_DIR, VAL_U_CACHE_DIR, num_grid_r=NUM_GRID_R, num_grid_c=NUM_GRID_C, num_chunk=NUM_CHUNK, num_his=num_his)

    # Step 2: 划分训练/验证集
    # ds = ds.train_test_split(test_size=0.1, seed=42)
    train_ds = ds
    eval_ds = vds.shuffle(seed=42).select(range(int(len(vds) * 0.2)))

    # 将 vds 和 vuds 的一定比例数据加入训练集以增强训练
    from datasets import concatenate_datasets
    vds_sampled = vds.shuffle(seed=42).select(range(int(len(vds) * augment_ratio)))
    vuds_sampled = vuds.shuffle(seed=42).select(range(int(len(vuds) * augment_ratio_u)))
